...
{
    return &view->providedBuffer_->bufferResource_->memory_;
}
...
void Root::DefineShader(ShaderKey const& key, const char* relativePath, VKW::ShaderModuleType type)
{
    Shader& shader = shaderMap_[key];
    shader.shaderModule_ = shaderModuleFactory_->LoadModule(relativePath, type);
}
...
void Root::DefineGraphicsPipeline(PipelineKey const& key, GraphicsPipelineDesc const& desc)
{
    ...
    for (std::uint32_t i = 0u; i < desc.shaderStagesCount_; ++i) {
        ...
        vkwDesc.shaderStages_[i] = shader.shaderModule_;
    }
...
void Root::CopyStagingBufferToGPUBuffer(ResourceKey const& src, ResourceKey const& dst, std::uint32_t context)
{
    ...
    VKW::BufferResource* srcBuffer = srcView->providedBuffer_->bufferResource_;
    VKW::BufferResource* dstBuffer = dstView->providedBuffer_->bufferResource_;
...
void Root::CreateImageFromFile(IOManager& ioManager, ResourceKey const& uploadBuffer, ResourceKey const& imageKey, std::string const& sourceFile, VkFormat format, Data::TextureChannelVariations channels, VkImageLayout targetLayout, VKW::ImageUsage usage)
{
    ...
    ImageLayoutTransition(0, 1, &imageView->resource_->handle_, &imageLayout);
}
