    ...
    , blurSetLayout_{ "mxlt"}
{
    ...
    VKW::ImageResource* sceneColorBufferResource = sceneColorBufferView->resource_.GetResource();
    ...
    VKW::ImageResource* maskUploadImageResource = maskUploadImageView->resource_.GetResource();
    ...
    VKW::ImageResource* colorBufferResource = colorBufferImageView->resource_.GetResource();
    ...
    for (std::uint32_t i = 0; i < framesCount; ++i)
    {
        ...
        VKW::ImageResource* blurBufferImageResource = blurBufferImageView->resource_.GetResource();
...
void CustomTempBlurPass::Apply(std::uint32_t contextId, VKW::WorkerFrameCommandReciever* commandReciever)
{
    ...
    VKW::ImageResource* blurBufferResource = blurBufferView->resource_.GetResource();
